...
import com.dnb.ubo.v3.calculation.ResponseCalculation;
...
import com.dnb.ubo.v3.jsonbeans.AddressCountry_;
import com.dnb.ubo.v3.jsonbeans.AddressCountry__;
import com.dnb.ubo.v3.jsonbeans.ApiResponse;
import com.dnb.ubo.v3.jsonbeans.Beneficiary;
import com.dnb.ubo.v3.jsonbeans.BeneficiaryAddress;
import com.dnb.ubo.v3.jsonbeans.BeneficiaryType;
import com.dnb.ubo.v3.jsonbeans.BusinessEntityType_;
import com.dnb.ubo.v3.jsonbeans.BusinessEntityType__;
...
import com.dnb.ubo.v3.jsonbeans.MemberAddress;
import com.dnb.ubo.v3.jsonbeans.MemberType;
import com.dnb.ubo.v3.jsonbeans.NodeSummaryByGeography;
import com.dnb.ubo.v3.jsonbeans.Organization;
import com.dnb.ubo.v3.jsonbeans.OwnershipStructure;
import com.dnb.ubo.v3.jsonbeans.OwnershipSummary;
import com.dnb.ubo.v3.jsonbeans.OwnershipUnavailableReason_;
import com.dnb.ubo.v3.jsonbeans.ProductResponse;
import com.dnb.ubo.v3.jsonbeans.Relationship;
import com.dnb.ubo.v3.jsonbeans.TransactionInfo_;
import com.dnb.ubo.v3.jsonbeans.TransactionStatus;
...
public class JsonBuilder {
    ...
    public static final ResponseWrapper getBeneficialOwnershipStructureResponse(final Map<String, Object> reqDetails, final Map<String, Object> nodesAndRelationshipList, final List<Map<String, Object>> linkageValuesList)
        ...
        if ((C.OWNERSHIPERR).equals(methodToInvoke)) {
            ...
            final Map<String, Map<Long, Object>> resultOfCalculation = responseCalculation.calculatebeneficialOwnershipList(nodesAndRelationshipList);
        ...
        populateInquiryDetail(inquiryDetail, reqDetails, reqDunsNbr);
    ...
    private static final List<String> populateRelations(final Map<String, Object> dbData) {
        ...
        for (int i = C.ZERO; i < relationshipValueList.size(); i++) {
            ...
            relation = relationId + "-" + fromId + "-" + toId + "-" + share;
    ...
    private static final void populateTargetNotPresentResponse(final ApiResponse apiResponse, final ProductResponse productResponse, final TransactionStatus transactionStatus, final Map<String, Object> reqDetails) {
        ...
        // Log error
        LOGGER.error(C.ERROR + C.VERSIONV3 + "  " + C.NOTARGETDUNS + " " + C.DUNS + " " + reqDetails.get(C.DUNSNBR) + " " + C.TRANSACTION_ID + " " + reqDetails.get(C.TRANSACTION_ID));
    }
    ...
    private static final void populateTargetPresentNoDataResponse(final ApiResponse apiResponse, final ProductResponse productResponse, final TransactionStatus transactionStatus, final Map<String, Object> reqDetails,
        ...
        // Log Error
        LOGGER.error(C.ERROR + C.VERSIONV3 + "  " + C.NOOWNERSHIPDATA + " " + C.DUNS + " " + reqDetails.get(C.DUNSNBR) + " " + C.TRANSACTION_ID + " " + reqDetails.get(C.TRANSACTION_ID));
     ...
     * @throws ApplicationException
     */
    public static final List<Member> getMemberList(final List<Node> internalNodes, final Map<String, Map<Long, Object>> dbResponseListValue, final String dunsNbr, final List<Node> hmoNodeList) {
        ...
        for (Node node : internalNodes) {
            ...
            if (null != nodeMap) {
                ...
                setAdditionalMemberDetails(member, dbResponseListValue, node, hmoNodeList);
            }
    ...
    public static final ResponseWrapper getBeneficialOwnershipListResponse(final Map<String, Object> reqDetails, final Map<String, Object> nodesAndRelationshipList, final List<Map<String, Object>> linkageValuesList)
        ...
        if ((C.OWNERSHIPERR).equals(methodToInvoke)) {
            ...
            // Invoke Calculation
            final Map<String, Map<Long, Object>> calculationResponse = responseCalculation.calculatebeneficialOwnershipList(nodesAndRelationshipList);
            responseCalculation = null;
     ...
     * @throws ApplicationException
     */
    public static final List<Beneficiary> populateBeneficiary(final Map<String, Map<Long, Object>> dbResponseListValue, final List<Node> internalNodes, final String dunsNbr, final List<Node> hmoNodeList)
                    throws ApplicationException {
        ...
        for (Node node : internalNodes) {
            ...
            if (null != nodeMap) {
                ...
                // Other beneficiary Details. Added new list for identifying
                // nodes with outgoing HAS_MAJORITY_OWNER
                setBeneficiaryDetails(nodeMap, beneficiary, dbResponseListValue, node, beneficiaryAddress, hmoNodeList);
        ...
        JsonBuilderHelper.removeTargetDuns(beneficiaryList, dunsNbr);
    ...
    private static final void exceptionForUnprocessableEntity(final TransactionStatus transactionStatus, final String code, final String text, final String value) {
        if (null != text && (text.contains(C.PRODUCT_CODE) || (text.contains(C.DUNS)) || (text.contains(C.TRANSACTION_ID)))) {
            ...
            transactionStatus.setStatusMessage(C.MISSINGPARAMETER + " " + value);
    ...
    private static final void exceptionForFilters(final TransactionStatus transactionStatus, final String text, final String code) {
        if (null != text && text.contains(C.INVALID_BOOLEAN_VALUE)) {
            transactionStatus.setStatusMessage(C.MISSINGPARAMETER + " includeUnknownHoldings/excludeUndisclosedHoldings");
            transactionStatus.setStatusCode(C.UB012);
            transactionStatus.setIsFailure(true);
        } else if (null != text && text.contains(C.DOUBLE)) {
            transactionStatus.setStatusMessage(C.MISSINGPARAMETER + " ownershipPercentage");
            transactionStatus.setStatusCode(C.UB012);
            transactionStatus.setIsFailure(true);
     ...
     * @param hmoNodeList
     */
    public static final void setAdditionalMemberDetails(final Member member, final Map<String, Map<Long, Object>> dbResponseListValue, final Node node, List<Node> hmoNodeList) {
        ...
        // Added new list for identifying nodes with outgoing HAS_MAJORITY_OWNER
        benPerForCBOMember(member, node, beneficialSum, hmoNodeList);
     ...
     * @param hmoNodeList
     */
    public static final void benPerForCBOMember(final Member member, final Node node, final Map<Long, Object> beneficialSum, List<Node> hmoNodeList) {
        ...
        for (Long key : keySet) {
            if (key < C.ZERO) {
                ...
                // Added new check for identifying nodes with outgoing
                // HAS_MAJORITY_OWNER
                if ((keyAbs.equals(node.id())) && (null != member.getOwnershipUnavailableReason() && !member.getOwnershipUnavailableReason().isEmpty() || hmoNodeList.contains(node.id()))) {
     ...
     * @param hmoNodeList
     */
    private static final void setAdditionalBeneficiaryDetails(final Beneficiary beneficiary, final Map<String, Map<Long, Object>> dbResponseListValue, final Node node, List<Node> hmoNodeList) {
        final Map<Long, Object> indirect = dbResponseListValue.get(C.INDIRECT);
        final Map<Long, Object> direct = dbResponseListValue.get(C.DIRECT);
        final Map<Long, Object> beneficialSum = dbResponseListValue.get(C.BENEFECIALSUM);
        final Map<Long, Object> isBeneficiary = dbResponseListValue.get(C.ISBENFECIARY);
        ...
        // Added new list for identifying nodes with outgoing HAS_MAJORITY_OWNER
        setBenPerForBenefeciary(beneficiary, node, beneficialSum, hmoNodeList);
     ...
     * @param hmoNodeList
     */
    public static final void setBenPerForBenefeciary(final Beneficiary beneficiary, final Node node, final Map<Long, Object> beneficialSum, List<Node> hmoNodeList) {
        ...
        for (Long key : keySet) {
            if (key < C.ZERO) {
                ...
                // Added new check for identifying nodes with outgoing
                // HAS_MAJORITY_OWNER
                if ((keyAbs.equals(node.id())) && (null != beneficiary.getOwnershipUnavailableReason() && !beneficiary.getOwnershipUnavailableReason().isEmpty() || hmoNodeList.contains(node.id()))) {
    ...
    private static final void setBeneficiaryDetails(final Map<String, Object> nodeMap, final Beneficiary beneficiary, final Map<String, Map<Long, Object>> dbResponseListValue, final Node node,
                    final BeneficiaryAddress beneficiaryAddress, List<Node> hmoNodeList) throws ApplicationException {
        ...
        try {
            ...
            // Added new check for identifying nodes with outgoing
            // HAS_MAJORITY_OWNER
            setAdditionalBeneficiaryDetails(beneficiary, dbResponseListValue, node, hmoNodeList);
    ...
    public static final TransactionStatus populateProductResponse(final ProductResponse productResponse, final Map<String, Map<Long, Object>> calculationResponse, final List<Map<String, Object>> linkageValues,
                    final Map<String, Object> reqDetails, final Map<String, Object> nodesAndRelationshipList) throws ApplicationException {

        // Fetch NodeList
        final List<Member> memberList = getMemberList((List) nodesAndRelationshipList.get(C.DETAILS), calculationResponse, (String) reqDetails.get(C.DUNSPADDED), (List) nodesAndRelationshipList.get(C.HMO_NODES));
        ...
        OwnershipSummary ownershipSummary = getOwnershipSummary(memberList, relationshipList.size(), (String) reqDetails.get(C.DUNSPADDED));
    ...
    public static final TransactionStatus populateListResponse(final Map<String, Map<Long, Object>> calculationResponseMap, final List<Map<String, Object>> linkageValuesList, final ProductResponse productResponse,
        ...
        beneficiaryList = populateBeneficiary(calculationResponseMap, (List) nodesAndRelationshipList.get(C.DETAILS), (String) reqDetails.get(C.DUNSPADDED), (List) nodesAndRelationshipList.get(C.HMO_NODES));
        ...
        if (beneficiaryList.isEmpty()) {
            ...
            // log error
            LOGGER.error(C.ERROR + C.VERSIONV3 + "  " + C.FILTERNOOWNERSHIP + " " + C.DUNS + " " + reqDetails.get(C.DUNSNBR) + " " + C.TRANSACTION_ID + " " + reqDetails.get(C.TRANSACTION_ID));
        } else {
            // Set beneficiary
    ...
    /**
     * To populate inquiry details
     *
     * @param inquiryDetail
     * @param reqDetails
     */
    public static final void populateInquiryDetail(final InquiryDetail inquiryDetail, final Map<String, Object> reqDetails, final String reqDunsNbr) {
        inquiryDetail.setDuns(reqDunsNbr);
        if (null != reqDetails.get(C.IND_COUNTRY) && !((String) reqDetails.get(C.IND_COUNTRY)).isEmpty()) {
            inquiryDetail.setIsoAlpha2CountryCode((String) reqDetails.get(C.IND_COUNTRY));
        }
        if (null != reqDetails.get(C.OWNERSHIP_TYPE) && !((String) reqDetails.get(C.OWNERSHIP_TYPE)).isEmpty()) {
            inquiryDetail.setOwnershipType((String) reqDetails.get(C.OWNERSHIP_TYPE));
        }
        if (null != reqDetails.get(C.OWNERSHIP_PERCENTAGE) && (Double) reqDetails.get(C.OWNERSHIP_PERCENTAGE) != 0.0) {
            inquiryDetail.setOwnershipPercentage((Double) reqDetails.get(C.OWNERSHIP_PERCENTAGE));
        }
        inquiryDetail.setProductCode(Integer.valueOf((String) reqDetails.get(C.PRODUCT_CODE)));
        inquiryDetail.setIncludeUnknownHoldings((Boolean) reqDetails.get(C.INCLUDE_UNKNOWN_HOLDINGS));
        inquiryDetail.setExcludeUndisclosedHoldings((Boolean) reqDetails.get(C.EXCLUDE_UNDISCLOSED_HOLDINGS));
    ...
    private static final void populateOwnershipSummaryforList(final List<Beneficiary> beneficiaryList, final Organization organization, final int relationSize) {
        ...
        for (Beneficiary beneficiary : beneficiaryList) {
            ...
            // Unknown count
            unknownCount = getUnknownCount(countryMap, unknownCount, beneficiary);

            // CBO
            if (beneficiary.getIsBeneficiary() && null != beneficiary.getBeneficiaryType().getCode() && C.MEBR_TYPE_CODE_BUSINESS == beneficiary.getBeneficiaryType().getCode()) {
                cboCount++;
            }
        ...
        // Country summary
        JsonBuilderHelper.setCountrySummary(geographySum, countryMap, unknownCount);
    ...
    /**
     * To set summary details for country
     *
     * @param Map
     ...
     * @return int
     */
    private static final int getUnknownCount(final Map<String, Integer> countryMap, final int unknownCount, final Beneficiary beneficiary) {

        int countryUnknown = unknownCount;
        if (null != beneficiary.getBeneficiaryAddress() && null != beneficiary.getBeneficiaryAddress().getAddressCountry() && null != beneficiary.getBeneficiaryAddress().getAddressCountry().getIsoAlpha2Code()) {

            String country = beneficiary.getBeneficiaryAddress().getAddressCountry().getIsoAlpha2Code();
    ...
    public static final OwnershipSummary getOwnershipSummary(final List<Member> memberList, int relationSize, String targetDuns) {
        ...
        for (Member member : memberList) {
            ...
            if (member.getIsBeneficiary() && null != member.getMemberType().getCode() && C.MEBR_TYPE_CODE_BUSINESS == member.getMemberType().getCode()) {
                cboCount++;
            }
            ...
            unknownCount = JsonBuilderHelper.getCountryCount(countryMap, unknownCount, member, targetDuns);
        }
