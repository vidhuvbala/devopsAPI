...
import com.dnb.ubo.v3.constants.Query;
...
public class DaoImpl implements Dao {
    ...
    public ResponseWrapper getBeneficiaryOps(final Map<String,Object> reqDetails, final String operationFlag) throws ApplicationException {
        ...
        try {
            ...
            Map<String, Object> nodesAndRelationshipList = fetchNodeAndRelationship(dunsNbr);
    ...
    public  final List<Map<String, Object>> fetchCorporateLinkage(final String inputDunsNumber) throws ApplicationException {
        ...
        // put params
        final Map<String, Object> params = setDUNSForQuery(inputDunsNumber);
        ...
        // invoke response
             final StatementResult rsForCorporateLinkage = session.readTransaction(new TransactionWork<StatementResult>() {
                 ...
                 public StatementResult execute(Transaction tx) {
                     StringBuilder query = new StringBuilder(Query.CORPORATE_LINKAGE_QUERY_1+Query.CORPORATE_LINKAGE_QUERY_2);
                     // invoke query
    ...
    * Method for Fetching Beneficiary
     ...
     * @throws ApplicationException
     */
    public  final Map<String, Object> fetchNodeAndRelationship(final String inputDunsNumber) throws ApplicationException {
        // initialize
        ...
        if (rsForList.hasNext()) {
            Record record = rsForList.next();
                ...
                // Added in Sprint 20 for backward compatibility
                if (null != record.get(C.HMO_NODES)) {
                    data.put(C.HMO_NODES, record.get(C.HMO_NODES).asList());
                }
        }
     ...
     * @return
     */
    private static final Map<String, Object> setDUNSForQuery(String inputDunsNumber) {
