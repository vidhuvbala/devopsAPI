 ...
 */
package com.dnb.ubo.v4.calculation;
...
public class ResponseCalculation {
    ...
    private final Map<Long, Object> beneficialSum, direct, indirect, unDisclosed, depth, isBeneficiary,orgBen;
     ...
     */
    public Map<String, Map<Long, Object>> calculatebeneficialOwnershipList(Map<String, Object> dbData,boolean maxDeptAval) {
    ...
    /**
     * To identify CBO in case max_depth is given as request param
     *
     * @return List<Long>
     */
    public void markOrgBenef() {
        List<Long> x = new ArrayList<Long>();
        allRel.forEach((k, v) ->
                    x.add(new Long(k.substring(0, k.lastIndexOf("|"))))
        );
        organisations.forEach((k, v) -> {
            if (!x.contains(k)) {
                orgBen.put(k,C.ZEROPER);
            }
        });
