 ...
 */
package com.dnb.ubo.v4.request;
...
public class RequestValidator {
    ...
    public static final boolean requestValidator(final Map<String, Object> reqDetails, final String operationFlag) throws ApplicationException {
        ...
        if (checkDuns(invalid, (String) reqDetails.get(C.DUNSPADDED))) {
            ...
            invalid = validationforFilter(reqDetails, invalid);
        }
        if (invalid) {
     ...
     * @throws ApplicationException
     */
    private static boolean validationforFilter(final Map<String, Object> reqDetails, boolean invalid) throws ApplicationException {
        boolean isValid = invalid;
        if (excludeUndisclosedHoldingsforStructure((String) reqDetails.get(C.PRODUCT_CODE), (Boolean) reqDetails.get("excludeUndisclosedHoldings"))) {
            isValid = true;
        } else if (checkDepth(reqDetails.get(C.DEPTH))) {
            throw new ApplicationException("Invalid depth", C.UB012);
        }
        return isValid;
    }
     ...
     * @return boolean
     */
    public static boolean checkDepth(final Object depth) {
        boolean invalid = false;
        String depthStr = "";
        if (null != depth) {
            depthStr = depth.toString();
            if (!Pattern.compile(C.REGEX_DEPTH).matcher(depthStr).matches()) {
                invalid = true;
            }
        }
        return invalid;
