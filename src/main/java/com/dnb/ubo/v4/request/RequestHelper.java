...
public class RequestHelper {
    ...
    public static final InquiryDetail inquriydetails(HttpServletRequest request) {
        ...
        try {
            ...
            populatePerDepth(request, inqdetls);
        } catch (Exception e) {
    ...
    /**
     * Split the function inquriydetails due to sonar
     * @param request
     * @param inqdetls
     */
    public static void populatePerDepth(HttpServletRequest request, InquiryDetail inqdetls) {
        ...
        if (isEntityInRequest(request, C.DEPTH) && (Integer.parseInt(request.getParameter(C.DEPTH)))!=C.ZERO) {
            inqdetls.setDepth(Integer.parseInt(request.getParameter(C.DEPTH)));
        }
    }
    ...
    public static final void setHttpCodes(ApiResponse uboresponse, HttpServletResponse response) {
        ...
        if (C.UB003.equals(uboresponse.getTransactionStatus().getStatusCode())) {
