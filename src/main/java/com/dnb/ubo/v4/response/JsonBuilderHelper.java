...
import com.dnb.ubo.v4.jsonbeans.BusinessEntityType;
import com.dnb.ubo.v4.jsonbeans.Confidence;
import com.dnb.ubo.v4.jsonbeans.Confidence_;
import com.dnb.ubo.v4.jsonbeans.Confidence__;
import com.dnb.ubo.v4.jsonbeans.ControlType;
import com.dnb.ubo.v4.jsonbeans.ControlType_;
import com.dnb.ubo.v4.jsonbeans.ControlType__;
import com.dnb.ubo.v4.jsonbeans.CorporateLinkage;
import com.dnb.ubo.v4.jsonbeans.DomesticUltimate;
import com.dnb.ubo.v4.jsonbeans.GlobalUltimate;
...
import com.dnb.ubo.v4.jsonbeans.OwnershipUnavailableReason;
import com.dnb.ubo.v4.jsonbeans.Parent;
import com.dnb.ubo.v4.jsonbeans.PrimarySIC;
...
public class JsonBuilderHelper {
    ...
    public static final void populateTargetDunsDetails(final Node targetDunsValue, final Organization organization) {
        ...
        if (null != nodeMap) {
            ...
            if (nodeMap.containsKey(C.CONTROL_TYPE)) {
                populateOwnershipDetials(organization, nodeMap);
            }
    ...
    /**
     * Populate Control type details in Organization.
     *
     * @param organization
     *            the organization
     * @param string
     *            the string
     */
    private static void populateOwnershipDetials(Organization organization, Map<String, Object> nodeMap) {

        ControlType controlType = new ControlType();

        if (nodeMap.containsKey(C.CONFIDENCE_CD)) {
            Confidence confidence = new Confidence();
            String confidenceCode = (nodeMap.get(C.CONFIDENCE_CD)).toString();
            confidence.setCode(Integer.parseInt(confidenceCode));
            controlType.setConfidence(confidence);
        }

        String controlTypeCode = (nodeMap.get(C.CONTROL_TYPE)).toString();
        controlType.setCode(Integer.parseInt(controlTypeCode));

        organization.setControlType(controlType);
    }
    ...
    /**
     * Sets the member control type.
     *
     * @param nodeMap the node map
     * @param member the member
     */
    public static void setMemberControlType(final Map<String, Object> nodeMap, final Member member) {

        ControlType_ controlType = new ControlType_();

        if (nodeMap.containsKey(C.CONFIDENCE_CD)) {
            Confidence_ confidence = new Confidence_();
            String confidenceCode = (nodeMap.get(C.CONFIDENCE_CD)).toString();
            confidence.setCode(Integer.parseInt(confidenceCode));
            controlType.setConfidence(confidence);
        }
        ...
        member.setControlType(controlType);
    }
    ...
    public static final List<Beneficiary> filterOperation(final List<Beneficiary> beneficiaryList, final Boolean excludeUndisclosedHoldings, final String ownershipType, final Double ownershipPercentage) {
        ...
        if (null != ownershipPercentage && ownershipPercentage != 0 && !beneficiaryFilteredList.isEmpty()) {
            beneficiaryFilteredList = JsonBuilderSupport.ownershipPercentageFilter(ownershipType, ownershipPercentage, beneficiaryFilteredList, excludeUndisclosedHoldings);
    ...
    private static final List<Beneficiary> ownershipTypeFilter(final String ownershipType, final List<Beneficiary> beneficiaryList, final Boolean excludeUndisclosedHoldings) {
        ...
        if (null == excludeUndisclosedHoldings || !excludeUndisclosedHoldings) {
            beneficiaryFilteredList.addAll(JsonBuilderSupport.getUndisclosedOnes(beneficiaryList));
        }
     ...
     */
    public static final void filterOperationforRelationship(final List<Member> memberList, final List<com.dnb.ubo.v4.jsonbeans.Relationship> relationshipList) {
     ...
     * @return int
     */
    public static final int getUnknownCount(final Map<String, Integer> countryMap, final int unknownCount, final Beneficiary beneficiary) {
    ...
    /**
     * Sets the control type summary in list.
     *
     * @param List
     *            <Beneficiary> beneficiaryList
     ...
     *            ownershipSummary
     */
    public static void setControlTypeSummary(final List<Beneficiary> beneficiaryList, OwnershipSummary ownershipSummary) {
        ...
        for (Beneficiary beneficiary : beneficiaryList) {
            if (null != beneficiary.getControlType() && null != beneficiary.getControlType().getCode()) {
            if (beneficiary.getControlType().getCode() == C.STATE_OWNED_ENTERPRISE) {
                stateOwned++;
            }
            if (beneficiary.getControlType().getCode() == C.GOVT_ENTITY) {
                govtEntity++;
            }
            if (beneficiary.getControlType().getCode() == C.PUBLICLY_TRADED) {
                publicComp++;
            }
            if (beneficiary.getControlType().getCode() == C.PRIVATELY_OWNED) {
                privateComp++;
            }
        }
        }
    ...
    /**
     * Sets the benefecial control type .
     *
     * @param nodeMap
     *            the node map
     * @param beneficiary
     *            the beneficiary
     */
    public static void setBenefecialControlType(Map<String, Object> nodeMap, Beneficiary beneficiary) {

        ControlType__ controlType = new ControlType__();
        ...
        if(nodeMap.containsKey(C.CONTROL_TYPE)){
            ...
            if (nodeMap.containsKey(C.CONFIDENCE_CD)) {
                Confidence__ confidence = new Confidence__();
                String confidenceCode = (nodeMap.get(C.CONFIDENCE_CD)).toString();
                confidence.setCode(Integer.parseInt(confidenceCode));
                controlType.setConfidence(confidence);
            }
        ...
        beneficiary.setControlType(controlType);
    }
    ...
    public static final void populateInquiryDetail(final InquiryDetail inquiryDetail, final Map<String, Object> reqDetails, final String reqDunsNbr) {
        ...
        if (null != reqDetails.get(C.DEPTH)) {
            inquiryDetail.setDepth((Integer) reqDetails.get(C.DEPTH));
        }
    ...
    public static final String getErrorvalue(final String text, final String value) {
        ...
        if (text.contains(C.DUNS)) {
            ...
            errorValue = getErrorforDtDepth(text, errorValue);
        }
        return errorValue;
    }

    /**
     * Splitting the getErrorvalue function for sonar
     *
     * @param text
     * @param errorValue
     * @return
     */
    public static String getErrorforDtDepth(final String text, String errorValue) {
        String valueErr = errorValue;
        if (text.contains(C.INVALID_DATE)) {
            valueErr = C.TRANSACTION_TIME_STAMP;
        } else if (text.contains(C.DEPTH)) {
            valueErr = C.DEPTH;
        }
        return valueErr;
    }
