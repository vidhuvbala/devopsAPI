...
public class JsonCleaner {
     ...
     */
     static void removeEmptyFieldforbeneficiaries(ObjectMapper mapper, List<Beneficiary> beneficiaries) {
        ...
        for (Beneficiary beneficiary : beneficiaries) {
            ...
            if (isEmptyField(mapper, beneficiary.getControlType())) {
                beneficiary.setControlType(null);
            }
     ...
     */
     static void removeEmptyFieldforMembers(ObjectMapper mapper, List<Member> members) {
        ...
        for (Member member : members) {
            ...
            if (isEmptyField(mapper, member.getControlType())) {
                member.setControlType(null);
            }
    ...
    private static void removeEmptyFieldsForOrganization(ObjectMapper mapper, Organization organization) {
        ...
        removeOrgAddControlTypeEmptyField(mapper, organization);

    }

    /**
     * Removes the organisation address &  control type empty fields.
     *
     * @param mapper the mapper
     * @param organization the organization
     */
    public static void removeOrgAddControlTypeEmptyField(final ObjectMapper mapper, final Organization organization) {
        if (null != organization.getAddress()) {
            removeEmptyforOrganisationAddress(mapper, organization);
        }
        if(null != organization.getControlType() && isEmptyField(mapper,organization.getControlType())){
            organization.setControlType(null);
        }
    }
